...
if (!empty($_POST['q_name'])) {
  $sql_search = " where a_user like '%" . $_POST['q_name'] . "%' "; // like '%keyword%'
} else {
  $sql_search = '';
}

//get total rows
$rs = $conn->query("select count(a_id) as num from tbl_admin $sql_search "); // query แบบมีเงื่อนไข ถ้ามีการส่งค่าค้นหา
...
<div class="table-responsive shadow mb-3 ">
  <table class="table">
    ...
    <tr>
      ...
      <td bgcolor="5b59a1" style="color: white;">บัญชีผู้ใช้</td>
      <td bgcolor="5b59a1" style="color: white;">รหัสผ่าน</td>
      <td bgcolor="5b59a1" style="color: white;">ชื่อ</td>
      <td bgcolor="5b59a1" style="color: white;">อีเมล</td>
    ...
    ?>
        <tr>
          ...
          <td> <a href="#" onclick="edit(<?php echo $row['a_id']; ?>);" data-toggle="modal" data-target="#dataModal" class='btn btn-outline-warning btn-lg font1 shadow p-1' style="width: 50px;height: 38px;"><i class='fa fa-cog'></i> </a></td>
          <td> <a href="#" onclick="javascript:del('<?php echo $row['a_id']; ?>');" class='btn btn-outline-danger btn-lg shadow p-1' style="width: 50px;height: 38px;"><i class="fa fa-bitbucket"></i> </a></td>
        </tr>
